...
El objetivo es exponer **Penpot** a través del proxy inverso centralizado `edugonmor_nginx`, asignándole el subdominio `penpot.edugonmor.com`.
...
Seguir el patrón existente en `edugonmor_nginx`.
1.  **Configuración**: Crear `config/conf.d/penpot.conf`.
2.  **Red**: Añadir `edugonmor_penpot_network` a `docker-compose.yml` de Nginx.
...
Tras tu aprobación:
1.  **Nginx**:
    - Crear `config/conf.d/penpot.conf` (Proxy a `edugonmor_penpot_frontend:80`).
    - Modificar `docker-compose.yml` para incluir la red `edugonmor_penpot_network`.
2.  **Verificación**:
