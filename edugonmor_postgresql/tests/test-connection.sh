...
if docker exec edugonmor_postgresql_services pg_isready -U "$POSTGRES_USER" >/dev/null 2>&1; then
    echo "✓ OK"
else
...
RESULT=$(docker exec edugonmor_postgresql_services psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -tAc "SELECT version();" 2>/dev/null)
if [ -n "$RESULT" ]; then
    echo "✓ OK"
...
EXT_COUNT=$(docker exec edugonmor_postgresql_services psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -tAc "SELECT count(*) FROM pg_available_extensions;" 2>/dev/null)
if [ -n "$EXT_COUNT" ]; then
