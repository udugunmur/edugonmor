#!/bin/bash
#
# configure-vnc-server.sh - Configure VNC remote access server (x11vnc)
#
# This script configures x11vnc to allow remote connections to the desktop session.
# IMPORTANT: Disables Wayland to ensure compatibility with x11vnc.
#
# Usage: sudo ./configure-vnc-server.sh
#
# Documentation:
#   - x11vnc: https://github.com/LibVNC/x11vnc
#   - GDM Wayland: https://help.gnome.org/admin/system-admin-guide/stable/login-automatic.html.en
#
...
disable_wayland() {
    ...
    elif grep -q "^WaylandEnable=false" /etc/gdm3/custom.conf 2>/dev/null; then
...
configure_headless_mode() {
    # Necessary when no physical monitor is connected
