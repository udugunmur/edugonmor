# Arquitectura del Sistema

Este documento describe la arquitectura del sistema tools.

## 📋 Visión General

```
┌─────────────────────────────────────────────────────────────────┐
│                    tools                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │              tools_network                     │   │
│  │                                                          │   │
│  │  ┌──────────────────────┐  ┌──────────────────────────┐ │   │
│  │  │  Portainer CE        │  │  Backup Service          │ │   │
│  │  │  (services)          │  │  (backup)                │ │   │
│  │  │                      │  │                          │ │   │
│  │  │  📊 Dashboard        │  │  🕐 Cron Job (03:00 AM)  │ │   │
│  │  │  🐳 Docker Mgmt     │  │  📦 Tar Compress         │ │   │
│  │  │  📈 Monitoring       │  │  🗑️ Retention (10 days)  │ │   │
│  │  │                      │  │                          │ │   │
│  │  │  Ports: 9443, 9000   │  │                          │ │   │
│  │  └──────────────────────┘  └──────────────────────────┘ │   │
│  │          │                           │                   │   │
│  └──────────┼───────────────────────────┼───────────────────┘   │
│             │                           │                       │
│  ┌──────────▼──────────┐     ┌──────────▼──────────┐           │
│  │ tools_    │     │ tools_    │           │
│  │ volumen             │────▶│ backups             │           │
│  │ (Datos Portainer)   │     │ (Rclone Mount)      │           │
│  └─────────────────────┘     └─────────────────────┘           │
│                                        │                        │
└────────────────────────────────────────┼────────────────────────┘
                                         │
                              ┌──────────▼──────────┐
                              │  rclone   │
                              │  (Cloud Sync)       │
                              └─────────────────────┘
```

## 🔧 Componentes

### 1. Portainer CE Service

| Aspecto | Detalle |
|---------|---------|
| **Imagen** | `portainer/portainer-ce:latest` |
| **Contenedor** | `tools_services` |
| **Puertos** | 9443 (HTTPS), 9000 (HTTP) |
| **Volúmenes** | Docker socket, datos persistentes |
| **Función** | Gestión visual de contenedores Docker |

### 2. Backup Service

| Aspecto | Detalle |
|---------|---------|
| **Imagen** | `alpine:3.19` |
| **Contenedor** | `tools_backup` |
| **Programación** | Diario a las 03:00 AM |
| **Retención** | 10 días |

### 3. Red

| Aspecto | Detalle |
|---------|---------|
| **Nombre** | `tools_network` |
| **Driver** | `bridge` |

### 4. Volúmenes

| Volumen | Tipo | Descripción |
|---------|------|-------------|
| `tools_volumen` | Docker Volume | Datos de Portainer |
| `tools_backups` | Bind Mount | Conexión a Rclone |

## 🔐 Seguridad

- `no-new-privileges:true`: Evita escalada de privilegios
- Red aislada: Comunicación solo dentro de la red definida
- Contraseña hasheada: bcrypt para credenciales
