...
run_test "Contenedor Portainer healthy" "docker inspect tools_services --format='{{.State.Status}}' | grep -q running"
...
echo ""
echo "--- Docker Socket ---"
run_test "Socket montado correctamente" "docker exec tools_services ls -la /var/run/docker.sock"

echo ""
echo "--- Volumen de Datos ---"
run_test "Directorio /data existe" "docker exec tools_services ls -la /data"
