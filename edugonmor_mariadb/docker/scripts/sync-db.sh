...
for ((i=0; i<$USERS_COUNT; i++)); do
    ...
    mariadb -u root -p"$(cat $PASSWORD_FILE)" -e "CREATE USER IF NOT EXISTS '$USERNAME'@'%' IDENTIFIED BY '$PASSWORD';"
    ...
    for ((j=0; j<$DBS_COUNT; j++)); do
        ...
        mariadb -u root -p"$(cat $PASSWORD_FILE)" -e "CREATE DATABASE IF NOT EXISTS \`$DBNAME\`;"
        mariadb -u root -p"$(cat $PASSWORD_FILE)" -e "GRANT ALL PRIVILEGES ON \`$DBNAME\`.* TO '$USERNAME'@'%';"
    done
done

echo "Flushing privileges..."
mariadb -u root -p"$(cat $PASSWORD_FILE)" -e "FLUSH PRIVILEGES;"
echo "Done."
